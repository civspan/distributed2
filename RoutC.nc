...
implementation
{
  ...
  /* Battery level */
  uint16_t battery = 0;
   /* Weight of current router */
   uint16_t router_weigth = 0;
   ...
   */
	void announceReceive(rout_msg_t *mess) {
		 ...
		 */
			...
			int16_t my_dist		= distance(TOS_NODE_ID);			/* Distance to sink */
			int16_t ancr_dist 	= distance(mess->from);				/* Distance between announcer and sink */
			int16_t btryweight	= ( 4 - ( mess->btrylvl / 25 ) );	/*  */
			int16_t	send_cost	= distanceBetween(TOS_NODE_ID, mess->from); /* Cost to send to announcer */
			int16_t dist_dif	= ancr_dist - my_dist;
			int16_t weight;
			if (mess->btrylvl < 10){
			//	btryweight++;
				btryweight = 13 - mess->btrylvl;
			}
			weight = ancr_dist + btryweight + send_cost/2;
			if(router == -1 && my_dist > ancr_dist ){
				router = mess->from;
				router_weigth = weight;
			}
			else if( router_weigth > weight ) { 
					router = mess->from;
					router_weigth = weight;
			}
		}
